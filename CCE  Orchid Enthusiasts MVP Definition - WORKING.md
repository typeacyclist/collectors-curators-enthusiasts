...
> **Status (2026-08-01):** The product and experience requirements in this document are current.
> The runtime and deployment architecture is governed by **ADR-001** and **ADR-003** in
> `docs/architecture/` — the Firebase-native serverless stack. The original Google Cloud Platform
> hosting section below (FastAPI on Cloud Run with Cloud SQL) has been retired and replaced with a
> pointer. See `docs/CODE-REVIEW.md` for background.
...
The MVP is delivered first as a PWA and must remain compatible with Capacitor iOS and Android shells without changing the underlying CCE contracts or Thing records. Separate native codebases are not required.
...
> **Retired.** The original hosting section specified a FastAPI service on Cloud Run backed by
> Cloud SQL for PostgreSQL. That runtime is no longer the target and has been removed to keep this
> document accurate. See **ADR-003** (`docs/architecture/ADR-003-retire-fastapi-adopt-firebase-native.md`)
> for the decision and **ADR-001** (`docs/architecture/ADR-001-ai-native-mobile-offline.md`) for the
> target stack. The prior text remains recoverable in git history.

The runtime target is the Firebase-native serverless stack: a React/TypeScript PWA on Firebase
Hosting, Firebase Authentication and App Check, Cloud Firestore with Security Rules (offline SDK),
Cloud Storage, Cloud Functions (TypeScript) for trusted commands, and Genkit/Vertex AI for governed
AI. Tenant isolation is enforced by Firestore Security Rules plus server-side membership checks in
callable commands. See ADR-001 for the full service list and ADR-003 for the migration plan.
